...
## Quickstart
The beta release is focused on **Registry** and **Provenance**. The minimal example below creates a file, appends rows to a registry, iterates decoded rows, and inspects recent provenance events. See the [tutorials](docs/tutorials) for more detailed instruction of how to use the `dnastream` package.
