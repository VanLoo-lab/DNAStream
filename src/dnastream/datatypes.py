...
SAMPLE_DTYPE = np.dtype([
                    ...
                    ("bam_file", h5py.string_dtype(encoding="utf-8")),  # Single-character strin
])
...
DATASET_LOG_DTYPE = np.dtype([
            ("timestamp", h5py.string_dtype(encoding="utf-8")),  # Timestamp
            ("dataset", h5py.string_dtype(encoding="utf-8")),   # Dataset path modified
            ("operation", "S15"),  # Operation type: "add", "update", "delete"
            ("user", "S15"),  # User who performed the modification
            ("hostname", "S15"),  # Host machine
            ("file", h5py.string_dtype(encoding="utf-8")),  # Host machine
])