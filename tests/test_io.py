...
def test_init_io_object(dnastream_obj):
    ...
    assert myio.ds.sample is not None
    assert len(myio.ds.sample) == 0
    assert len(myio.ds.variant) == 0
    assert myio.ds.variant is not None


def test_add_variants_from_maf(dnastream_obj, temp_maf):
    assert len(dnastream_obj.variant) == 0
...
def test_add_empty_file(dnastream_obj, temp_csv_file):
